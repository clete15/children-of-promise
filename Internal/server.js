...
const STAFF_SEED = [
    ...
    { name: 'Renee Nier', role: 'Assistant Teacher', program: 'PI', classroom: 'Toddlers / 2 Year Olds', fte: '1.0', education: '', eceCredentials: '', gateways: '', experienceYears: '', registryId: '', notes: 'Sole staff member in the Toddlers / 2 Year Olds room as of 2026-2027. Not in Gateways report as of 2/24/2025 — qualifications still need gathering.' },
...
// days — the conservative capacity-planning assumption that a child needs every day.
function parseRequestedDays(text) {
    const s = String(text || '').toLowerCase();
    const days = [
        /\bmon(day)?\b|\bm\b|(^|[^a-z])m([^a-z]|$)/.test(s),
        /\btue(s|sday)?\b|\bt\b|(^|[^a-z])t([^a-z]|$)/.test(s),
        /\bwed(nesday)?\b|\bw\b|(^|[^a-z])w([^a-z]|$)/.test(s),
        /\bthu(r|rs|rsday)?\b|\bth\b/.test(s),
        /\bfri(day)?\b|\bf\b|(^|[^a-z])f([^a-z]|$)/.test(s),
    ];
    if (days.some(Boolean)) return days.map(d => d ? 1 : 0);
